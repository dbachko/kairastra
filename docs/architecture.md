...
- `rust/src/workflow.rs`: loads `WORKFLOW.md` and parses prompt plus front matter
...
- `rust/src/setup.rs`: generates workflow, env, and systemd scaffolding for operators
...
1. `run` loads the workflow and validates it into `Settings`.
...
- `setup` fails if the generated files do not pass `doctor`.
